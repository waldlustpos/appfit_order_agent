...
import 'package:appfit_order_agent/screens/label_subinfo_settings_screen.dart';
...
/// 설정 좌측 패널의 라벨 프린터 카드 안에 들어가는 두 항목
/// — "라벨 출력 카테고리 지정" + "라벨 서브정보 표시".
///
...
class _SettingsLabelOutputItemsState
  ...
  Widget build(BuildContext context) {
    ...
    final groups = ref.watch(shopOptionGroupListProvider).value ?? const [];
    final prefs = ref.watch(preferenceServiceProvider);
    final selectedKeys = prefs.getLabelCategoryKeys();
    final selectedGroups = prefs.getLabelSubInfoGroups();
    ...
    return Column(
      children: [
        ...
        SettingsItemWidget(
          title: t.settings.label_subinfo.title,
          description: _subInfoDescription(groups, selectedGroups),
          isVertical: true,
          showDivider: false,
          trailing: ElevatedButton.icon(
            onPressed: () => _open(const LabelSubInfoSettingsScreen()),
            icon: const Icon(Icons.text_fields, size: 18),
            label: Text(t.settings.label_subinfo.btn_configure),
            style: AppStyles.primaryButton(),
          ),
        ),
      ],
  ...
  String _subInfoDescription(
    List<ShopOptionGroupModel> groups,
    List<String> selectedGroups,
  ) {
    if (selectedGroups.isEmpty) return t.settings.label_subinfo.desc_none;
    // 고른 순서 그대로 나열한다 — 그 순서가 곧 인쇄 순서다.
    final names = [
      for (final code in selectedGroups)
        groups
            .firstWhere(
              (g) => g.groupCode == code,
              orElse: () => ShopOptionGroupModel(
                groupCode: code,
                groupName: '',
                displayOrder: 0,
              ),
    ...
    return t.settings.label_subinfo.desc_selected(names: names.join(' → '));
  }
}
