...
- 미픽업 처리(READY → NOT_PICKED_UP. 고객이 찾아가지 않은 주문을 취소(환불)와 **구분해** 종결. **서버 스펙 논의 중 — 엔드포인트·상태 문자열·소켓 이벤트명 3개 미확정, 가칭으로 배선하고 그대로 노출(404 면 에러 다이얼로그)**. 교체 지점은 `kNotPickedUpServerStatus`/`kNotPickedUpServerAliases`/`_kNotPickedUpTransport` 3곳뿐. 새 터미널 상태는 진행도 격자가 아니라 `kTerminalStatusPriority` 에 넣어야 폴링 부활을 막는다. 배포되면 ARCHITECTURE.md 로 흡수하고 삭제): [docs/ORDER_NOT_PICKED_UP.md](docs/ORDER_NOT_PICKED_UP.md)
