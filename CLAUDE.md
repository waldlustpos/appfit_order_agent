...
- 패키지: `co.kr.waldlust.order.receive.appfit` (Tier 0 공통) / `co.kr.waldlust.order.receive.appfit.mammoth` (Tier 1 매머드 전용). **국가는 빌드를 가르지 않는다** — 서버 live/japanLive/staging 은 로그인 화면에서 런타임 선택되고, **릴리즈 빌드에 한해** 매장 ID 프리픽스로 자동 전환된다(개발 빌드는 명시 선택이 정본 — 아티팩트별 차이 없음). 빌드를 가르는 축은 브랜드 아티팩트 티어 하나뿐이며, 사정거리는 applicationId·런처 label/icon·OTA 채널까지다(`lib/config/build_brand.dart`).
...
- 미픽업 처리(READY → NO_SHOW. 고객이 찾아가지 않은 주문을 취소(환불)와 **구분해** 종결. `PUT /v0/order/{orderNo}` + `action: NO_SHOW` — 기존 상태변경 엔드포인트와 같아 `updateOrderStatus` 로 흡수했다(`forceCompleteOrder` 와 반대 판단, 근거는 문서 §2). **앱 구현 완료·실기기 검증 대기·미배포**. 서버가 **외부 이벤트를 발행하지 않아** 타 기기는 폴링으로만 수렴한다. action 어휘(`NO_SHOW`)와 응답 status 어휘는 별개 축이라 상수를 겸용하면 안 되며, 응답 status 원문은 스테이징 실측 대기(틀리면 '취소'로 조용히 표시된다). 새 터미널 상태는 진행도 격자가 아니라 `kTerminalStatusPriority` 에 넣어야 폴링 부활을 막는다. 배포되면 ARCHITECTURE.md 로 흡수하고 삭제): [docs/ORDER_NO_SHOW.md](docs/ORDER_NO_SHOW.md)
- 매장 자동 CLOSED 제안(서버측 dead-man switch. 정전·플러그 뽑힘·하드 리셋·네트워크 단절은 **클라이언트가 원리적으로 감지 불가**라 서버가 소켓 부재 N분으로 판정해야 한다는 제안서. 앱 구현 없음·서버 결정 대기. 클라이언트가 덮는 4경로는 이미 구현돼 있고 전원종료 경로의 실측치도 이 문서에 있다): [docs/STORE_AUTO_CLOSE.md](docs/STORE_AUTO_CLOSE.md)
