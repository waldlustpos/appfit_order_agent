...
- **일본향 Simple POS 기초 조사**(신규 앱 착수 판단용 사실 조사. 서버는 AppFit 단독 — POS 웹 어드민·POS 서버·CFG 체계 전면 배제, 부족분은 AppFit 신규 구현 전제. 결제는 Square Terminal API. **2027-04 일본 소비세 개정(식료품 8%→1%, 각의결정·법안 미성립)이 최우선 변수 — 세율 하드코딩 금지**. §8 은 재사용 자산이 못 덮는 POS 고유 영역 — 오프라인 완결 판매·로컬 원장·영업일 마감·현금 관리·금액 계산 결정론·크래시 원자성. §9 는 UI/UX — Simple 6원칙·1280×800 레이아웃·취식/포장 3중 표시·이 앱 디자인 토큰의 재사용 판정표, 그리고 **번들 Pretendard 에 일본어 한자가 없어 ja 문자열 85%가 폰트 혼합 렌더링이라는 실측**(이 앱에도 해당). 이 앱의 코드 변경 계획서가 아니라 별도 신규 앱의 조사 문서. **정본은 `appfit_simple_pos/docs/SIMPLE_POS_JP.md` 로 이관됐고 여기엔 포인터만 남아 있다** — 파일럿은 그 레포에서 진행 중이며 진행 상황 정본은 그쪽 `docs/ROADMAP.md`): [docs/SIMPLE_POS_JP.md](docs/SIMPLE_POS_JP.md)
