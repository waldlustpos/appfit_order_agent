...
| Layer 3 — 동작 seam | Strategy/Hook 인터페이스 | 동작이 갈리는 소수 지점만 분리: `LabelFilterStrategy`(TPCP 라벨 메뉴 필터), `SoundGraphHook`(매머드 자동접수 후 전송), `qrPayloadStrategyProvider`(라벨 QR 페이로드) — 비대상 브랜드는 NoOp |

### 3.2 등록 브랜드 (prefix 표)

한 브랜드가 프리픽스를 여러 개 가질 수 있다(`BrandMeta.prefixEnvironments`,
`Map<프리픽스, 서버환경>`) — 매머드가 유일한 사례.
...
- 빌드 아티팩트 티어(런타임과 별개 축): 대부분 Tier 0(공통), 승격 조건 3개를 전부 충족하면 Tier 1(전용 아티팩트) — 현재 매머드가 유일. 상세: [RELEASE.md](RELEASE.md), [BUILD_VARIANTS.md](BUILD_VARIANTS.md).
...
분리한다 — 현재 매머드(`….appfit.mammoth` / `appfit_order_agent_mammoth.exe`)
...
| Windows 채널(매머드) | `appfit_order_agent_mammoth_windows_version.json` / `appfit_order_agent_mammoth_windows.zip` |
| 병존 설치 | Tier 0 는 머신당 1개(재설치 시 in-place 업그레이드). Tier 1(매머드)은 별도 applicationId/exe명/설치 GUID 라 **Tier 0 와 같은 머신에 병존 설치 가능** |
