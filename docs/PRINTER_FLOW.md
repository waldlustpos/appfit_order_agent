...
`떼기대기 (앞 라벨을 안 뗌 — 비프음 울림)` 분기가 3회 정상 진입해 각각 9.1s / 8.0s / 5.6s
뒤 완료됐다 — **보류·해제 메커니즘 자체는 설계대로 동작한다.**
...
비프음은 펌웨어로 낼 수 없다는 것이 확정됐으므로, 알림이 필요하면 **앱이 내야 한다**
(`SoundService` + audioplayers 기존 자산). 다만 소리가 프린터가 아닌 **본체**에서 나므로
매장 배치(라벨=USB 별개 본체)에 따라 유용성이 갈린다 — 별도 판단 사항.

무코드로 남은 하드웨어 확인 하나: **진짜 에러(커버 열기/용지 제거)에서 소리가 나는지.**
난다면 버저 하드웨어는 있고 이 조건에만 안 쓰는 것이라 유틸리티/펌웨어 설정 여지가 있고,
어느 상황에서도 무음이면 버저 자체가 없는 것이다.

#### 아직 검증 안 된 것

① 진짜 커버열림/용지소진 회귀 ② `상태정상`(BASIC variant 폴백) 도달 여부 ③ REXOD 무영향
④ 이상 프레임 재현 시 가드 동작.
