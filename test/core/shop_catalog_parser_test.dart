...
void main() {
  ...
  group('옵션그룹 추출 (라벨 서브정보 설정의 후보 정본)', () {
    test('같은 그룹이 상품마다 반복 등장해도 1건으로 접히고 첫 이름이 정본이다', () {
      final shot = _option(id: 'opt-shot', posId: 'POS-SHOT', name: '샷 추가');
      final data = _data([
        _category(posId: 'DX0000', name: '커피', items: [
          _item(
            ...
            name: '아메리카노',
            optionGroups: [
              _group(posId: 'TKP001', name: '온도', options: [shot]),
            ],
          ),
          _item(
            shopItemId: 'item-2',
            ...
            name: '라떼',
            optionGroups: [
              // 같은 POS 코드에 다른 이름 — 첫 등장 이름을 유지한다.
              _group(posId: 'TKP001', name: '온도(수정중)', options: [shot]),
              _group(posId: 'TKP004', name: '사이즈', options: [shot]),
            ],
          ),
        ]),
      ]);

      final groups = parseShopCatalog(data).optionGroups;

      expect(groups.map((g) => g.groupCode), ['TKP001', 'TKP004']);
      expect(groups.first.groupName, '온도');
      expect(groups.last.groupName, '사이즈');
    });

    test('POS 코드가 없는 그룹은 스킵한다 (주문 응답과 조인할 수 없다)', () {
      final data = _data([
        _category(posId: 'DX0000', name: '커피', items: [
          _item(
            shopItemId: 'item-1',
            ...
            name: '아메리카노',
            optionGroups: [
              _group(posId: '', name: '이름만 있는 그룹', options: [
                _option(id: 'opt-1', posId: 'POS-O1', name: 'A'),
              ]),
            ],
          ),
        ]),
      ...
      final result = parseShopCatalog(data);
      expect(result.optionGroups, isEmpty);
      // 그룹은 못 써도 옵션 자체는 종전대로 수집된다.
      expect(_options(result.products), hasLength(1));
    });

    test('이름이 없으면 코드가 표시명이 된다 (고를 수는 있어야 한다)', () {
      final data = _data([
        _category(posId: 'DX0000', name: '커피', items: [
          _item(
            shopItemId: 'item-1',
      ...
      final group = parseShopCatalog(data).optionGroups.single;
      expect(group.groupName, isEmpty);
      expect(group.displayName, 'TKP009');
    });

    test('categories 가 없으면 빈 목록', () {
      expect(parseShopCatalog(<String, dynamic>{}).optionGroups, isEmpty);
    });
  });
