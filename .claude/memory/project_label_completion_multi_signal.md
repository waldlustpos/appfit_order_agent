...
metadata: 
  ...
  modified: 2026-08-07T01:27:46.154Z
---
...
| 상황 | 결과 |
|---|---|
| 정상 | `출력끝 (1213ms, 프린터응답 880ms)` — 슬라이싱해도 응답 유실 없음 |
| 보류→떼기 | `출력끝 (12645ms, 라벨나옴 12515ms)` — edge 후 **481ms** |
| 계속 안 뗌 | 30.2초에 `떼기대기 (앞 라벨을 안 뗌 — 비프음 울림)` — 폴백·비프음 유지 |
| 실패(강제) | `실패 [완료신호 없음 30초 — 재시도 금지] (…) 프린터=작업중 라벨=없음 연결=정상 \| pg=… 비콘[…] 동기[…]` |

peel edge 는 query 응답보다 일관되게 150~500ms 빠르다.

## 로그 grep 키워드 (2026-08-07 우리말화 이후)

`프린터응답` = query 신호로 완료 / `라벨나옴` = peel edge 로 완료 → 둘의 비율이 신호별 기여도.
`완료신호 없음` = 진짜 ACK 실패. 실패 줄은 `요약 | 원자료` 두 층이고 요약은 `프린터=/라벨=/연결=`.
⚠️ 구 표기 `via=query` / `via=peel` / `ACK timeout` / `PAPERNOFETCH` 로는 안 잡힌다.
성공 줄의 `pg=` 와 Dart 의 `ACK 미수신` 파일 로그는 중복이라 제거됐다(콘솔·Sentry 는 유지).
