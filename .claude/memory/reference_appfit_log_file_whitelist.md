...
**규칙:**
1. 파일에 남으려면 **`level >= Level.warning`** 이거나 라인에 **태그 문자열**(`[PLATFORM]`/`[SYSTEM]`/`[UI_ACTION]`/`[WEBSOCKET]`/`[LIFECYCLE]`/`[Notifier]`)이 포함돼야 한다. `LogTag.STATE`/`STORAGE`/`NAVIGATION` 등 화이트리스트에 없는 태그는 `logToFile` 을 써도 파일에서 사라진다.
