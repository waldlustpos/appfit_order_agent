...
- [라벨 카테고리 필터+subInfo 매장설정화](project_label_category_subinfo_settings.md) — TPCP 하드코딩 완전 대체(시드 없음 → TPCP 현장 재설정 필요). ON+선택0개=전량출력, fail-open (2026-09-03)
- [매장 범위 키의 매장 ID 정본](reference_active_store_id_is_session_key.md) — getActiveStoreId(). getId() 단독은 '아이디 저장' 종속이라 빌 수 있음
