...
metadata: 
  ...
  modified: 2026-08-05T07:01:29.841Z
---

완전 신규 설치(매장 ID·KEY_ENVIRONMENT 둘 다 없음) 기기에서 로그인화면 서버선택 초기값이 무조건 `'live'`(한국)로 고정되던 것을, 기기 타임존으로 국가를 추정해 유도하도록 구현 완료(2026-08-05, main 브랜치 uncommitted).
...
**남은 것**: `flutter analyze` 통과만 확인, 실기기(Android 타임존 변경 + 완전 초기화, Windows 제어판 타임존 변경) 검증 미실시. 커밋도 안 함.
