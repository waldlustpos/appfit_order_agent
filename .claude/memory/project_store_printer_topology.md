...
- **라벨 프린터**: USB 직결, 지원 2기종 — REXOD RXLA-561(Caysn autoreplyprint SDK, CP_Port_OpenUsb, 갭 라벨) + BIXOLON G30(UPOS/JavaPOS SDK, 연속용지+커터, Android 전용 — Windows 미이식). Caysn D2/D3 는 화이트리스트 잔류. VID:PID 자동감지 라우팅(NativeMethodHandler, G30 판정이 좁아 먼저). 구 BIXOLON XD5-40d 는 2026-09-01 지원 종료 — 남은 BIXOLON 자산의 소유권은 [[project-bixolon-xd5-removal-residue]].
