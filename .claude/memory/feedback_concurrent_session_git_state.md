...
metadata: 
  ...
  modified: 2026-08-13T23:43:24.652Z
---
...
**2026-08-14 재발 — 이번엔 푸시까지 됐다.** USB Direct 라벨 작업 중 실기기 검증을
기다리느라 코드를 미커밋으로 두고 있었는데, 그 사이 다른 주체가 `중간커밋`(6dc5e85)
이라는 메시지로 내 6파일을 통째로 커밋하고 **origin 에 푸시**했다. 이전 사례는 로컬
커밋이라 amend 로 수습할 수 있었지만, 푸시된 뒤에는 공유 히스토리를 다시 쓰는 것이
되어 손댈 수 없다 → **의미 없는 메시지가 영구히 남는다.**

교훈: 실기기 검증 대기처럼 **미커밋 상태가 길어지는 구간이 곧 노출 구간**이다.
검증이 끝날 때까지 기다리지 말고, 코드가 컴파일·테스트를 통과한 시점에 먼저 커밋해
두는 편이 낫다(검증 결과는 후속 커밋의 주석/메모리로 붙이면 된다).

**How to apply:**
0. 실기기 왕복이 예상되면 **빌드가 통과한 즉시 커밋**한다. "검증 끝나면 한 번에"는
   동시 세션 환경에서 남에게 커밋을 빼앗기는 창을 열어 둔다.
