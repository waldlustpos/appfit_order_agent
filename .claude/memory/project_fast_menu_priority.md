...
metadata: 
  ...
  modified: 2026-08-14T05:35:50.679Z
---
...
## ★ 이 기능만으로는 처리량이 안 는다 (2026-08-14)

**순서를 바꿀 뿐 병목을 못 옮긴다.** 라벨 큐는 앞 라벨을 뗄 때까지 막히고,
`SerialAsyncQueue` 는 **이미 꺼내서 처리 중인 항목을 추월시키지 않는다.**
앞 주문이 종이를 안 떼고 있으면 우선순위와 무관하게 전부 대기한다.

그래서 고객사 요구("아아가 앞 주문에 막힌다")를 푸는 축은 셋이고, 이 기능은 그중
하나일 뿐이다:

| 안 | 축 | 처리량 | 상태 |
|---|---|---|---|
| 빠른메뉴 우선 | 시간(큐 순서) | **안 늚** | 이 메모 |
| 프린터 2대 | 공간(프린터) | 늚 | [[project_usb_direct_label_pipeline]] |
| KDS 쌍 추가 | 공간(단말) | 늚 | `localTargets` 로 이미 가능 |

**병목이 프린터 처리량이면 1안은 체감이 안 나온다.** 매장에 제안하기 전에 병목이
"큐가 밀린다" 인지 "프린터가 밀린다" 인지부터 가를 것.

## 구역과 이어짐 — 빠른메뉴 → 구역 직결 (b423985)

`LabelTargetPolicy.fastMenuTarget` 로 "빠른 메뉴는 구역 N 으로" 를 지정할 수 있다.
빠른메뉴 배정이 카테고리 배정을 **덮는다**(점이 면을 이긴다) — 카테고리 축만으로는
"아아만 따로" 라는 고객사 언어를 표현할 수 없기 때문.

⚠️ **구역이 나뉜 매장에서는 모드 2가 대체로 불필요해진다.** 빠른메뉴가 물리적으로
다른 큐로 가면 앞 주문의 떼기 대기에 애초에 안 막힌다. 모드 2는 굶주림 가드를 안고
가는 기능이므로, 구역 분리가 되면 **모드 1 + 구역 배정**이 권장 조합이고 모드 2는
프린터 1대 매장용으로 남긴다.
