---
description: Android 릴리즈 APK 빌드 후 Lightsail 서버에 배포 (japan / korea / 둘다 선택)
---
...
단일 패키지(`co.kr.waldlust.order.receive.appfit`)로 통합되었다. flavor 는 없으며,
국가는 `deploy_apk.sh` 인자 → `--dart-define=APPFIT_VARIANT` 로만 구분된다.
변형마다 **별도 OTA 채널**로 업로드된다:

| 변형 | 패키지 | 채널 파일 | 용도 |
| --- | --- | --- | --- |
| `japan` (기본) | `co.kr.waldlust.order.receive.appfit` | `appfit_order_agent_japan_version.json` / `_japan.apk` | 일본 매장 OTA(신규 채널). 신규 패키지로 재설치된 매장이 사용 |
| `korea` | `co.kr.waldlust.order.receive.appfit` | `appfit_order_agent_korea_version.json` / `_korea.apk` | 한국 신규 900매장 예정(아직 미배포) |

OTA URL 분기는 빌드 타임 `--dart-define=APPFIT_VARIANT` 로 결정된다 (`lib/config/ota_config.dart`).
...
- 현재 서버 버전: 변형에 해당하는 버전 JSON을 조회한다 (응답 `{"version": <int>}` = 현재 배포된 빌드번호):
  - japan: `curl -fsS --max-time 10 http://waldpay.kokonutstamp2.com/appfit_order_agent_japan_version.json`
  - korea: `curl -fsS --max-time 10 http://waldpay.kokonutstamp2.com/appfit_order_agent_korea_version.json`
...
선택에 따라 Bash 툴로 실행한다:
- japan: `./deploy_apk.sh japan`
- korea: `./deploy_apk.sh korea`
- 둘다: `./deploy_apk.sh japan` 실행·성공 확인 후 `./deploy_apk.sh korea`
