...
서빙한다. 변형 인자·`--dart-define=APPFIT_VARIANT` 는 없다(서버는 앱 로그인
화면에서 런타임 선택). Bash 툴로 실행:
```
./build_main.sh
```
...
- 빌드 성공분은 `build_main.sh` 가 **자동 아카이브**(`archive_apk.sh`)한다 — `!Project Files/appfit_order_agent/apk/<버전>/` 에 APK + `release_notes.txt` 보관 후 폴더가 열린다
