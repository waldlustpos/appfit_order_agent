...
- appfit 은 변형이 둘입니다: `update`(기본) / `standalone`.
  - update    → ZIP: `appfit_order_agent_windows.zip`, 패키지: `co.kr.waldlust.order.receive`
  - standalone → ZIP: `appfit_order_agent_standalone_windows.zip`, 패키지: `co.kr.waldlust.order.receive.appfit`
...
| 설치본 `.exe` | `build_installer.ps1` (`dist\AppfitOrderAgent[-Standalone]-Setup-<semver>.exe`) | 그대로 복사 |
...
# 설치본 보관 (standalone)
.\archive_windows.ps1 -SrcArtifact .\dist\AppfitOrderAgentStandalone-Setup-3.3.6.exe -Variant standalone
...
<ArchiveBase>\appfit_order_agent\windows\<버전>\
   ...
   └─ release_notes_update.txt            (또는 release_notes_standalone.txt)
```
